...
void odeint(const Vec_DP& y0, void f(double, const Vec_DP& y, Vec_DP& f),
            const Vec_DP& x, Mat_DP& Y, double eps)
// solve initial value problem
// input:
//   y0 = initial value of dependent variables y at x=x[0]
//   f = right hand side of differential equaiton dy/dx = f(x,y)
//   eps = error tolerance (Numerical Recipes, section 16.2)
//   x = data evaluation points
// output:
//   Y = dependent variables evaluated at x (shape(y0.size, x.size))
{
    int dum,i,j;
    double h;
    Vec_DP y(y0);
    kmax = 0;
    for(i=0; i<y.size(); i++) Y[i][0] = y[i];
    for(j=1; j<x.size(); j++) {
        h = (x[j] - x[j-1])*eps;
        odeint(y,x[j-1],x[j],eps,h,0,dum,dum,f,rkqs);
        for(i=0; i<y.size(); i++) Y[i][j] = y[i];
    }
}